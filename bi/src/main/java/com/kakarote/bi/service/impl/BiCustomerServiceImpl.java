...
public class BiCustomerServiceImpl implements BiCustomerService {
    ...
    public List<JSONObject> customerRecordStats(BiParams biParams) {
        ...
        if (userIds.size() == 0) {
            ...
            timeList.forEach(time -> recordList.add(new JSONObject()
                    .fluentPut("type", time)
                    .fluentPut("recordCount", 0)
                    .fluentPut("customerCount", 0)));
            return recordList;
        }
    ...
    public List<JSONObject> customerConversionStats(BiParams biParams) {
        ...
        if (userIds.size() == 0) {
            List<JSONObject> recordList = new ArrayList<>();
            timeList.forEach(time -> recordList.add(new JSONObject().fluentPut("type", time).fluentPut("pro", 0)));
            return recordList;
        }
        Map<String, Object> map = record.toMap();
        map.put("timeList", timeList);
    ...
    public List<JSONObject> poolStats(BiParams biParams) {
        ...
        if (userIds.size() == 0) {
            ...
            timeList.forEach(time -> recordList.add(new JSONObject()
                    .fluentPut("type", time)
                    .fluentPut("putInNum", 0)
                    .fluentPut("receiveNum", 0)));
            return recordList;
        }
        Map<String, Object> map = record.toMap();
        map.put("timeList", timeList);
    ...
    public JSONObject poolTable(BiParams biParams) {
        ...
        if (record.getUserIds().size() == 0) {
            return new JSONObject()
                    .fluentPut("list", new ArrayList<>())
                    .fluentPut("total", new JSONObject().fluentPut("realname", "总计").fluentPut("deptName", "").fluentPut("putInNum", 0).fluentPut("receiveNum", 0));
        }
